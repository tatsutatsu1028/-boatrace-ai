...
try:
    from threads_poster import (
        ...
        refresh_access_token as threads_refresh_access_token,
        token_age_days as threads_token_age_days,
    )
    THREADS_AVAILABLE = True
except Exception:
    THREADS_AVAILABLE = False
    threads_load_config = None
    threads_refresh_access_token = None
    threads_token_age_days = None
...
def _last_tracked_odds(race_date, jcd, rno):
    """対象レースの各comboについて最後に取得したオッズを返す。"""
    endpoint = (
    ...
    for row in rows:
        ...
        if combo and combo not in out:
            out[combo] = _safe_float(row.get("odds"), None)
    return out


def _rule_ticket_metrics(snapshot, official_result, last_odds):
    """
    研究ルールA/Bを仮想精算する。
    ...
    mainline = [
        ...
        and _safe_int(x.get("stake"), 0) > 0
    ]

    # A: 本命80%以上 + 本線のみ
    rule_a_eligible = p1_prob >= 0.80 and bool(mainline)
    a = settle(mainline) if rule_a_eligible else {"stake":0,"payout":0,"profit":0,"hit":False}

    # B: 本命70%以上 + 本線 + 締切側EV>=1.2
    rule_b_tickets = []
    if p1_prob >= 0.70:
        for x in mainline:
            combo = str(x.get("combo", "")).strip()
            ai_prob = _safe_float(x.get("prob"), 0.0)
            odd = last_odds.get(combo)
            if odd is not None and ai_prob * odd >= 1.20:
                rule_b_tickets.append(x)

    rule_b_eligible = bool(rule_b_tickets)
    b = settle(rule_b_tickets) if rule_b_eligible else {"stake":0,"payout":0,"profit":0,"hit":False}

    return p1_prob, rule_a_eligible, a, rule_b_eligible, b


def _upsert_research_rules(snapshot, race_date, jcd, rno, official_result):
    """研究ルールA/Bの結果を別テーブルへ保存する。"""
    ctx = _race_key(race_date, jcd, rno)
    last_odds = _last_tracked_odds(race_date, jcd, rno)

    p1_prob, a_ok, a, b_ok, b = _rule_ticket_metrics(
        snapshot,
        official_result,
        last_odds,
    )
    ...
    record = {
        ...
        "rule_b_hit": b["hit"],
        "last_odds_available": bool(last_odds),
    }
...
def _try_finalize_result(race_date, jcd, rno):
    ...
    # 研究ルールA/Bは本番購入ロジックとは完全に分離して仮想精算する。
    try:
...
def main():
    ...
    # Threadsトークンの自動更新
    # -------------------------------------------------
    # 長期トークンは60日で失効する。失効前に更新しておかないと、
    # ある日突然「投稿されていない」状態になる。
    # 5分おきに叩く必要はないので、前回更新から日数が経った時だけ実行する。
    try:
        _maybe_refresh_threads_token()
    except Exception as e:
        print(
            "[THREADS] トークン更新をスキップしました:",
            type(e).__name__,
            str(e),
            flush=True,
        )
...
# Threadsトークンを更新する間隔（日）。
# 有効期限60日に対して十分な余裕を取る。
THREADS_REFRESH_INTERVAL_DAYS = 20


def _maybe_refresh_threads_token():
    """前回更新から一定日数が経っていればThreadsトークンを更新する。"""
    if not THREADS_AVAILABLE:
        return

    cfg = threads_load_config(SUPABASE_URL, SUPABASE_KEY)
    if not cfg:
        # 未連携なら何もしない（毎回ログを出すとうるさいので黙る）。
        return

    age = threads_token_age_days(cfg)

    if age is not None and age < THREADS_REFRESH_INTERVAL_DAYS:
        return

    print(
        "[THREADS] トークンを更新します（前回更新から",
        "不明" if age is None else f"{age}日",
        "）",
        flush=True,
    )
    threads_refresh_access_token(SUPABASE_URL, SUPABASE_KEY)
