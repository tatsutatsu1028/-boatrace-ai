...
def predict(model, race, display_weight=0.32, current_meet_weight=0.18, course_weight=0.16, weather_weight=0.10, venue_course_weight=0.12):
    x = race.copy()
    ...
    # 展示Fの艇は、展示タイム・直線・まわり足・1周が（フライングした分）
    # 参考にならない・信頼できない値になりやすいため、加点・減点いずれの
    # 対象にもしない（「展示Fの場合だけ評価しない」という要望に対応）。
    # F自体を減点材料にはしない一方で、F艇の見かけ上速い展示タイムを
    # 誤って好材料として拾うことも防ぐ、という位置づき。
...
def assess_favorite_risk(race, final):
    ...
    # 本命艇が展示Fの場合、その展示走のタイムは（フライングした分）
    # 参考にならないため、展示関連のリスク評価（直線・まわり足・展示
    # タイム）はスキップする。Fそのものを減点材料にはしない
    # （「展示Fの場合だけ評価しない」という要望に対応）。
    fav_st = pd.to_numeric(pd.Series([fav.get("exhibition_st")]), errors="coerce").iloc[0]
    fav_is_flying = pd.notna(fav_st) and fav_st < 0

    if fav_is_flying:
        reasons.append("本命艇が展示F(展示データは評価対象外)")
    else:
