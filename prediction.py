...
from comment_analyzer import total_comment_score, analyze_comment_detail
...
def predict(model, race, display_weight=0.32, comment_weight=0.18, current_meet_weight=0.18, course_weight=0.16, weather_weight=0.10, venue_course_weight=0.12):
    x = race.copy()
    ...
    if "comment" in x:
        details = [analyze_comment_detail(v) for v in x["comment"]]
        cs = np.array([d["total"] for d in details], dtype=float)

        # コメントを重要材料として扱うが、1コメントだけで基礎能力を完全に覆さないよう上限を設定。
        comment_adj = np.clip(comment_weight * cs, -0.55, 0.55)
        adjustment += comment_adj
...
def assess_favorite_risk(race, final):
    ...
    展示・モーター/ボート・今節成績・コメント評価を総合したリスクを評価する。
    ...
    if isinstance(comment, str) and comment.strip():
        detail = analyze_comment_detail(comment)
        if detail["total"] <= -0.30:
            score += 1
            reasons.append("本命艇のコメントが弱気")

    return favorite_lane, score, reasons


def trifecta(first, gamma_b=0.8, gamma_c=0.65):
    """
    ...
    for a, b, c in itertools.permutations(range(1, 7), 3):
        ...
        pb = (s[b] ** gamma_b) / denom_b

        denom_c = sum(
            (v ** gamma_c) for k, v in s.items()
            if k not in (a, b)
        )
        pc = (s[c] ** gamma_c) / denom_c
