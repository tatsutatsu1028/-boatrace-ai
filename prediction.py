...
def rank_tickets(
    ...
    if has_odds:
        ev_for_rank = (
            x["expected_return"]
            .clip(lower=0, upper=2.50)
        )

        x["ev_rank"] = _safe_rank(ev_for_rank)
        x["odds_rank"] = _safe_rank(
            np.log1p(
                x["odds"].clip(lower=0)
            )
        )
    ...
    if has_odds:
        ...
        long_pool["long_score"] = (
            long_pool["ev_rank"] * 0.58
            + long_pool["odds_rank"] * 0.22
            + long_pool["prob_rank"] * 0.20
        )

        long_pool.loc[
            long_pool["prob_rank"] >= 0.90,
            "long_score",
        ] -= 0.12
