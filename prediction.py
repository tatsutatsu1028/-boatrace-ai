...
def predict(model, race, display_weight=0.32, current_meet_weight=0.18, course_weight=0.16, weather_weight=0.10, venue_course_weight=0.12):
    ...
    # 展示タイム（周回タイム）はSTとは別物で、フライングの有無に関わらず
    # 速さの参考になるため、F艇でも通常どおり評価する。
    # 直線（伸び足）・まわり足・1周は独自公開データのみで、Fの影響で
    # 見かけ上の数値が歪みやすいため、F艇はここだけ評価対象外にする。
    ...
    if "exhibition_st" in x:
        ...
        for i in range(len(x)):
            if flying.iloc[i]:
                reasons[i].append("展示F(直線・まわり足・1周は評価対象外)")
...
def assess_favorite_risk(race, final):
    ...
    # 本命艇が展示Fの場合、独自公開データである直線（伸び足）・まわり足は
    # （フライングした分）参考にならないためスキップする。展示タイム
    # （周回タイム）はSTとは別物で、Fの有無に関わらず参考になるため、
    ...
    if fav_is_flying:
        reasons.append("本命艇が展示F(直線・まわり足は評価対象外)")
    else:
