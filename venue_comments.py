...
MARUGAME_PDF = "https://www.marugameboat.jp/asp/marugame/pdf_marugame/pdf/{date}.pdf"

# 丸亀の出走表PDFは「印 枠 登番 選手名 …節間成績 選手コメント」という
# ヘッダの後に6艇分の行が続き、各行は
#   [枠番] [登番4桁] [氏名] …数値列… [コメント文]
# という並びになっている。改行位置はpypdf等の抽出結果でぶれるため、
# 空白類を単一スペースへ正規化した「1行化テキスト」に対して処理する。
_MARUGAME_ROW_PAT = re.compile(r"(?:^|\s)([1-6])\s+(\d{3,4})\s+")
_JP_TEXT_PAT = re.compile(r"[一-龥ぁ-んァ-ヶー、。][一-龥ぁ-んァ-ヶー、。\s]{6,}")


def fetch_marugame_comments(date_yyyymmdd, jcd, rno, race):
    out = _empty(race)
    url = MARUGAME_PDF.format(date=date_yyyymmdd)
    ...
    try:
        ...
        print("[MARUGAME PDF]", url, "pages=", len(reader.pages), "text_len=", len(raw_text))

    except Exception as e:
        print("[MARUGAME PDF ERROR]", type(e).__name__, str(e))
        return out

    flat = re.sub(r"\s+", " ", raw_text)

    # 「選手コメント」ヘッダはレースごとに1回出現するため、
    # rno番目のヘッダ以降・次の「フォーカス」までがそのレースの範囲。
    race_blocks = re.split(r"選手コメント", flat)

    rno_int = int(rno)
    if rno_int >= len(race_blocks):
        print("[MARUGAME PDF] race block not found:", rno_int, "blocks=", len(race_blocks) - 1)
        return out

    body = race_blocks[rno_int]
    end = body.find("フォーカス")
    if end >= 0:
        body = body[:end]

    rows = list(_MARUGAME_ROW_PAT.finditer(body))
    print("[MARUGAME PDF] rows=", len(rows))

    found = {}
    for i, m in enumerate(rows):
        lane = int(m.group(1))
        seg_end = rows[i + 1].start() if i + 1 < len(rows) else len(body)
        seg = body[m.end():seg_end]

        jp_matches = _JP_TEXT_PAT.findall(seg)
        if not jp_matches:
            continue

        comment = max(jp_matches, key=len)
        comment = re.sub(r"\s+", "", comment)
        comment = re.sub(r"^[ー―\-‐]+", "", comment)

        if comment and lane not in found:
            found[lane] = comment

    print("[MARUGAME PDF] entries=", len(found))

    for idx, row in race.iterrows():
        lane = int(row["lane"])
        comment = found.get(lane, "")

        ok = bool(comment) and 3 <= len(comment) <= 220 and not _looks_like_motor_table_noise(comment)
        if ok:
            out.loc[idx, "venue_comment"] = comment
            out.loc[idx, "venue_comment_source"] = "丸亀公式・出走表PDF"
            out.loc[idx, "venue_comment_url"] = url

    print(
        "[MARUGAME PDF] comments=",
        int((out["venue_comment"].astype(str).str.strip() != "").sum()),
    )
...
NARUTO_PDF = "https://www.n14.jp/uploads/cdn/pdf/z_yosou/{date}.pdf"

# 鳴門の前日予想PDFは「締切時刻 HH:MM」がレースの区切りで、各行が
#   [◎○△×(印、任意)] [枠番] [氏名(姓 名)] [級別A/B1-2] …数値列… [コメント文] [数値(任意)]
# という並びになっている。「123/456 …」という進入コース組み合わせの
# 行がテーブルの終端で、それ以降はレース展望の地の文なので取り込まない。
_NARUTO_RACE_PAT = re.compile(r"締切時刻\s*\d{1,2}:\d{2}")
_NARUTO_ROW_PAT = re.compile(
    r"(?:[◎○△×]\s*)?([1-6])\s+([一-龥々ぁ-んァ-ヶー]{1,4}\s+[一-龥々ぁ-んァ-ヶー]{1,4})\s+([AB][12])"
)
_NARUTO_ODDS_LINE_PAT = re.compile(r"\d+/\d+\s+\d")


def fetch_naruto_comments(date_yyyymmdd, jcd, rno, race):
    out = _empty(race)
    url = NARUTO_PDF.format(date=date_yyyymmdd)
    ...
    try:
        ...
        print("[NARUTO PDF]", url, "pages=", len(reader.pages), "text_len=", len(raw_text))

    except Exception as e:
        print("[NARUTO PDF ERROR]", type(e).__name__, str(e))
        return out

    flat = re.sub(r"\s+", " ", raw_text)

    race_positions = list(_NARUTO_RACE_PAT.finditer(flat))
    print("[NARUTO PDF] race markers=", len(race_positions))

    rno_int = int(rno)
    if rno_int > len(race_positions):
        print("[NARUTO PDF] race block not found:", rno_int, "found=", len(race_positions))
        return out

    m = race_positions[rno_int - 1]
    start = m.end()
    end = race_positions[rno_int].start() if rno_int < len(race_positions) else len(flat)
    body = flat[start:end]

    rows = list(_NARUTO_ROW_PAT.finditer(body))
    odds_m = _NARUTO_ODDS_LINE_PAT.search(body)
    table_end = odds_m.start() if odds_m else len(body)

    print("[NARUTO PDF] rows=", len(rows))

    jp_pat = re.compile(r"[一-龥々ぁ-んァ-ヶー、。][一-龥々ぁ-んァ-ヶー、。\s]{6,}")
    found = {}

    for j, rm in enumerate(rows):
        lane = int(rm.group(1))
        seg_end = rows[j + 1].start() if j + 1 < len(rows) else table_end
        seg_end = min(seg_end, table_end)
        seg = body[rm.end():seg_end]

        jp_matches = jp_pat.findall(seg)
        if not jp_matches:
            continue
    ...
    print("[NARUTO PDF] entries=", len(found))
    ...
    for idx, row in race.iterrows():
        ...
        if ok:
            out.loc[idx, "venue_comment"] = comment
            out.loc[idx, "venue_comment_source"] = "鳴門公式・前日予想PDF"
            out.loc[idx, "venue_comment_url"] = url
...
MIKUNI_PDF = "https://www.boatrace-mikuni.jp/uploads/cdn/pdf/z_yosou/{date}.pdf"

# 三国の前日予想PDFは鳴門と少し違うレイアウトで、レース区切りが
# 「締切時刻」ではなく締切時刻そのもの(HH:MM)が単独で現れる。行は
#   [印(任意)] [枠番] [登録番号] [氏名(姓 名)] [級別A/B1-2] …数値列… [コメント文] [数値(任意)]
# という並び。「3連単：」の行がテーブルの終端（それ以降は次レースの
# レース名見出し）。
_MIKUNI_TIME_PAT = re.compile(r"(?<!\d)(\d{1,2}:\d{2})(?!\d)")
_MIKUNI_ROW_PAT = re.compile(
    r"(?:[◎○△×X]\s*)?([1-6])\s+(\d{3,5})\s+([一-龥々ぁ-んァ-ヶー]{1,4}\s+[一-龥々ぁ-んァ-ヶー]{1,4})\s+([AB][12])"
)
_MIKUNI_TABLE_END_PAT = re.compile(r"3連単")


def fetch_mikuni_comments(date_yyyymmdd, jcd, rno, race):
    out = _empty(race)
    url = MIKUNI_PDF.format(date=date_yyyymmdd)
    ...
    try:
        ...
        print("[MIKUNI PDF]", url, "pages=", len(reader.pages), "text_len=", len(raw_text))

    except Exception as e:
        print("[MIKUNI PDF ERROR]", type(e).__name__, str(e))
        return out

    flat = re.sub(r"\s+", " ", raw_text)

    race_positions = list(_MIKUNI_TIME_PAT.finditer(flat))
    print("[MIKUNI PDF] race markers=", len(race_positions))

    rno_int = int(rno)
    if rno_int > len(race_positions):
        print("[MIKUNI PDF] race block not found:", rno_int, "found=", len(race_positions))
        return out
    ...
    rows = list(_MIKUNI_ROW_PAT.finditer(body))
    end_m = _MIKUNI_TABLE_END_PAT.search(body)
    table_end = end_m.start() if end_m else len(body)

    print("[MIKUNI PDF] rows=", len(rows))
    ...
    print("[MIKUNI PDF] entries=", len(found))
    ...
    for idx, row in race.iterrows():
        ...
        if ok:
            out.loc[idx, "venue_comment"] = comment
            out.loc[idx, "venue_comment_source"] = "三国公式・前日予想PDF"
            out.loc[idx, "venue_comment_url"] = url
...
# 「raceinfo-racers_comment」は唐津、「raceinfo-racer_comment」（単数形）は
# 芦屋で実在確認済み。「comment.php?r=」は若松のページ名として検索結果に
# 実在を確認（要検証）。「yosou-yosou」は津で実在確認済み
# （ただしそのままではコメント欄が見つからないこともある）。
# 鳴門(n14.jp)は唐津と同じページ命名規則（raceinfo-tenbo等）を実機確認済み。
COMMON_CMS_PAGE_CANDIDATES = (
    "index.php?page=raceinfo-racers_comment",
    "index.php?page=raceinfo-racer_comment",
    "comment.php?r={rno}",
...
VENUE_SOURCE_LABELS = {
    ...
    "12": "住之江公式", "13": "尼崎公式", "14": "鳴門公式", "17": "宮島公式", "18": "徳山公式",
    "19": "下関公式", "20": "若松公式", "21": "芦屋公式", "22": "福岡公式", "23": "唐津公式",
}
...
def fetch_venue_comments(date_yyyymmdd, jcd, rno, race):
    ...
    try:
        ...
        if code == "15":
            return fetch_marugame_comments(date_yyyymmdd, jcd, rno, race)

        if code == "14":
            return fetch_naruto_comments(date_yyyymmdd, jcd, rno, race)

        if code == "10":
            return fetch_mikuni_comments(date_yyyymmdd, jcd, rno, race)
