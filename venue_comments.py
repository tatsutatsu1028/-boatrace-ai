...
def _fetch_gamagori_pdf_comments(date_yyyymmdd, rno, race):
    ...
    race_positions = [
        ...
        for m in re.finditer(r"(\d{1,2})R(?=\s)", flat)
    ]
    ...
    if not race_positions:
        ...
        loose = [(m.start(), m.group(0)) for m in re.finditer(r"\d{1,2}R", flat)][:14]
...
TSU_COMMENT_PAGES = (
    # boatrace-tsu.com は「page=raceinfo-racers_comment」等の
    # 共通CMSページ名を持たないことを実機確認済み（404）。
    # 実在が確認できているのは元々使われていた「予想」ページのみ。
    "https://www.boatrace-tsu.com/sp/index.php?page=yosou-yosou&race={rno}",
)
...
def fetch_tsu_comments(date_yyyymmdd, jcd, rno, race):
    ...
    for url_tmpl in TSU_COMMENT_PAGES:
        url = url_tmpl.format(rno=int(rno))

        try:
            html = _get(url)
        except Exception as e:
            print("[TSU ERROR]", url, type(e).__name__, str(e))
            continue

        if _looks_like_no_data_page(html):
            print("[TSU] no data page:", url)
            continue

        found = _extract_comments_from_document(html, race)
        print("[TSU] comments=", len(found), "url=", url)
        ...
        for idx, row in race.iterrows():
            ...
            if c and _looks_like_natural_comment(c):
                out.loc[idx, "venue_comment"] = c
                out.loc[idx, "venue_comment_source"] = "津公式・全選手コメント"
                out.loc[idx, "venue_comment_url"] = url
