...
OMURA_COMMENT_ALL = "https://omurakyotei.jp/yosou/m/comment_all.php?day={date}"

# [選手名] コメント本文 という形式で1日分・全レース分の選手コメントが
# 並んでいるページ。レース番号での絞り込みは無いため、選手名で
# 該当艇にマッチさせる。同一選手が1日に複数回登場する場合は
# 最初に見つかったものを採用する（直前情報のため通常は1日1回）。
_OMURA_ENTRY_PAT = re.compile(r"\[([^\[\]]{2,20})\]\s*([^\[]+)")
...
def fetch_omura_comments(date_yyyymmdd, jcd, rno, race):
    ...
    found_by_name = {}
    for m in _OMURA_ENTRY_PAT.finditer(text):
        name = _compact_name(m.group(1))
        comment = _norm(m.group(2))
        # 次のエントリの手前まで拾ってしまう「モーター:数字 号機 過去のコメント」
        # 等の付随情報を取り除く。
        comment = re.split(r"モーター\s*[:：]", comment)[0].strip()
        comment = re.sub(r"過去の?コメント.*$", "", comment).strip()

        if name and comment and name not in found_by_name:
