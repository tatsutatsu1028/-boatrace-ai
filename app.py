        ...
        if st.button("🤖 AI最終予想", type="primary"):
            try:
                with st.spinner("AI解析中…"):
                    ...
                    pre["comment"] = ""
                    before = predict(model, pre, display_weight=0, comment_weight=0, weather_weight=0)
                    final = predict(model, work, display_weight=display_weight, comment_weight=comment_weight, weather_weight=weather_weight)
                    tri = trifecta(final)
