...
def _auth_cookie_manager():
    # CookieManager はブラウザCookieを読み書きするためのStreamlit component。
    # session_state に保持して同一セッション内での再生成を避ける。
    if "_boat_cookie_manager" not in st.session_state:
        st.session_state["_boat_cookie_manager"] = stx.CookieManager(
            key="boat_ai_auth_cookie_manager"
        )
    return st.session_state["_boat_cookie_manager"]
...
def _make_auth_token(role: str, expires_at: int, secret: str) -> str:
    payload = json.dumps(
        {"role": role, "exp": int(expires_at)},
        separators=(",", ":"),
        ensure_ascii=False,
    ).encode("utf-8")
...
def _verify_auth_token(token: str, secret: str):
    try:
        body, sig_text = str(token).split(".", 1)
        expected = hmac.new(
        ...
        if role not in {"admin", "staff"} or exp <= int(time.time()):
            return None
        return role
    except Exception:
        return None
...
def _login_gate():
    ...
    staff_pin = _secret_text("STAFF_A_PIN")
    cookie_secret = _secret_text("AUTH_COOKIE_SECRET")

    if not admin_pin or not staff_pin or not cookie_secret:
        st.error(
            "ログイン設定が未完了です。Streamlit Secrets に "
            "ADMIN_PIN / STAFF_A_PIN / AUTH_COOKIE_SECRET を登録してください。"
        )
        st.stop()

    cookie_manager = _auth_cookie_manager()
    cookie_name = "boat_ai_auth_v1"

    # 新しいStreamlitセッションになったときだけ、24時間Cookieから復元する。
    if st.session_state.get("auth_role") not in {"admin", "staff"}:
        token = cookie_manager.get(cookie_name)
        role = _verify_auth_token(token, cookie_secret) if token else None
        if role:
            st.session_state["auth_role"] = role
            st.session_state["collector_name"] = (
                "owner" if role == "admin" else "Aさん"
            )
    ...
    if st.session_state.get("auth_role") not in {"admin", "staff"}:
        ...
        if st.button("ログイン", key="login_button"):
            ...
            token = _make_auth_token(role, expires_at, cookie_secret)

            # PINそのものは保存しない。署名済みの役割＋期限だけを24時間保存する。
            cookie_manager.set(
                cookie_name,
                token,
                key=f"set_boat_ai_auth_{int(time.time() * 1000)}",
                path="/",
    ...
    collector = st.session_state.get("collector_name", "owner")
    st.caption(
        f"👤 {collector} / "
        ...
        try:
            if cookie_manager.get(cookie_name) is not None:
                cookie_manager.delete(
                    cookie_name,
                    key=f"delete_boat_ai_auth_{int(time.time() * 1000)}",
                )
        except Exception:
            pass
        st.rerun()
