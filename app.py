...
with tab1:
    ...
    if deadlines:
        st.markdown("#### ⏰ 締切予定時刻")
        deadline_grid = st.container()
        for start_r in range(1, 13, 4):
            dcols = deadline_grid.columns(4)
            for dcol, rr in zip(dcols, range(start_r, min(start_r + 4, 13))):
                hhmm = deadlines.get(rr, "--:--")
                with dcol:
                    st.markdown(
                        _deadline_html(rr, hhmm, d),
                        unsafe_allow_html=True,
                    )
        st.caption("締切30分以内のレースは時刻を強調表示します。")
    else:
        st.caption("締切予定時刻を取得できませんでした。レース選択は通常どおり使えます。")
