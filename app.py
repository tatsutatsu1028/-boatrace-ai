...
AUTH_COOKIE_NAME = "boat_ai_auth_v1"
...
def _auth_secret():
    ...
    fallback = _secret_text("ADMIN_PIN") + "|" + _secret_text("STAFF_A_PIN")
    return hashlib.sha256(fallback.encode("utf-8")).digest()


def _sign_auth(role, expires_at):
...
def _verify_auth(token):
    ...
    try:
        ...
        role, expires_s, sig = raw.split("|", 2)
...
def _role_to_collector(role):
    return "owner" if role == "admin" else "Aさん"
...
def _login_gate():
    ...
    if st.session_state.get("auth_role") not in {"admin", "staff"}:
        ...
        restored_role = _verify_auth(token)
        if restored_role:
            st.session_state["auth_role"] = restored_role
            st.session_state["collector_name"] = _role_to_collector(restored_role)
    ...
    if st.session_state.get("auth_role") not in {"admin", "staff"}:
        ...
        if st.button("ログイン", key="login_button"):
            ...
            collector = _role_to_collector(role)
            expires_ts = int(time.time()) + AUTH_TTL_SECONDS
            token = _sign_auth(role, expires_ts)
    ...
    collector = st.session_state.get(
        "collector_name", _role_to_collector(role)
    )
