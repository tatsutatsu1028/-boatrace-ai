...
# v1.10.9 mobile fixed deadline button grid
st.markdown("""
<style>
div[class*="st-key-deadline_button_row_"] [data-testid="stHorizontalBlock"]{
    display:flex !important;
    flex-wrap:nowrap !important;
    gap:4px !important;
}
div[class*="st-key-deadline_button_row_"] [data-testid="column"]{
    flex:1 1 25% !important;
    width:25% !important;
    min-width:0 !important;
}
div[class*="st-key-deadline_button_row_"] button{
    min-height:42px !important;
    padding:4px 1px !important;
}
div[class*="st-key-deadline_button_row_"] button p{
...
with tab1:
    ...
    # スマホで確実に4列×3段に収めるため、st.columns()を使わず
    # HTML/CSS Gridのリンクカードで表示する。
    # カードをタップすると query parameter を付けて同じページを開き、
    # 下の処理でそのRの公式データ取得まで自動実行する。
    picked_rno = None
    try:
        _picked = st.query_params.get("race_pick")
        if _picked not in (None, ""):
            picked_rno = int(_picked)
            if picked_rno not in range(1, 13):
                picked_rno = None
    except Exception:
        picked_rno = None

    # 同じRで再実行ループしないよう、取得トリガーとして1回だけ使う。
    fetch_requested = picked_rno is not None
    if fetch_requested:
        st.session_state["selected_rno"] = picked_rno
    ...
    grid_html = [
        ...
        .race-card-v1109{
            ...
            height:54px;
            padding:4px 1px;
            border:1px solid rgba(140,140,140,.34);
            border-radius:9px;
        ...
        .race-card-v1109 .rtime{
            margin-top:4px;
            font-size:11px;
            font-weight:800;
            white-space:nowrap;
        }
        .race-card-v1109.near .rtime{
            color:#e53935 !important;
            font-weight:900;
        }
        .race-card-v1109.selected{
            border-width:2px;
        }
        @media (max-width:480px){
            .race-grid-v1109{gap:4px;}
            .race-card-v1109{
                height:50px;
                border-radius:8px;
                padding:3px 0;
            }
            .race-card-v1109 .rno{font-size:11px;}
            .race-card-v1109 .rtime{font-size:10px;margin-top:3px;}
        }
        </style>
        <div class="race-grid-v1109">
        """
    ]
    ...
    for rr in range(1, 13):
        hhmm = deadlines.get(rr, "--:--") if deadlines else "--:--"
        mins = _deadline_minutes_left(d, hhmm)
        classes = ["race-card-v1109"]
        if mins is not None and 0 <= mins <= 30:
            classes.append("near")
        if current_selected == rr:
            classes.append("selected")

        # jcdを含めることで、カードタップ後も選択会場を明示的に維持。
        href = f"?race_pick={rr}&race_jcd={jcd}"
        grid_html.append(
            f'<a class="{" ".join(classes)}" href="{href}">'
            f'<span class="rno">{rr}R</span>'
            f'<span class="rtime">{hhmm}</span>'
            '</a>'
        )
    ...
    # カードのURLから会場も復元する。
    try:
        _picked_jcd = str(st.query_params.get("race_jcd") or "")
        if _picked_jcd in VENUES and _picked_jcd != st.session_state.get("selected_jcd"):
            st.session_state["selected_jcd"] = _picked_jcd
            jcd = _picked_jcd
    except Exception:
        pass
    ...
    if fetch_requested:
        try:
            ...
            st.query_params.pop("race_pick", None)
            st.query_params.pop("race_jcd", None)
