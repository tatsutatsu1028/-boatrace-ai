...
from prediction import train, predict, trifecta, rank_tickets, confidence, assess_favorite_risk
...
with tab3:
    ...
    st.divider()
    _style_options = ["コメント重視", "バランス", "展示重視"]
    _saved_style = saved.get("prediction_style", "コメント重視")
    ...
    hedge_enabled = st.toggle(
        ...
        help="本命買い目には毎回、本命艇（多くは1号艇）がどこかしらの着順で含まれがちです。展示・モーター/ボート・今節成績・コメントを総合して本命艇に不安要素が多いと判定された場合、穴の1点を本命艇を含まない組み合わせに差し替え、「本命艇が飛ぶ」リスクに備えます。",
    )
...
with tab4:
    ...
    if vm["races"] == 0:
        ...
        if detail_labels:
            ...
            try:
                lane_probs = json.loads(lane_probs_raw) if lane_probs_raw and pd.notna(lane_probs_raw) else []
            except Exception:
                lane_probs = []
...
with tab1:
    ...
    if race is not None:
        ...
        if st.button("🤖 AI最終予想", type="primary"):
            try:
                with st.spinner("AI解析中…"):
                    ...
                    pre["comment"] = ""
                    before = predict(model, pre, display_weight=0, comment_weight=0, weather_weight=0, venue_course_weight=0)
                    final = predict(model, work, display_weight=display_weight, comment_weight=comment_weight, weather_weight=weather_weight, venue_course_weight=venue_course_weight)
                    tri = trifecta(final)
        ...
        if result and result.get("context") == ctx:
            ...
            if "comment" in work_result.columns:
                merged = merged.merge(work_result[["lane", "comment"]], on="lane", how="left")
            ...
            for _, row in merged.iterrows():
                ...
                comment_txt = safe_name(row.get("comment", ""))

                if comment_txt:
                    grade = analyze_comment_detail(comment_txt)["grade"]
                    comment_html = (
                        f'<div class="small">🗣️ {grade} 「{html.escape(comment_txt)}」</div>'
                    )
                else:
                    comment_html = '<div class="small">🗣️ コメントなし</div>'
