...
# v1.10.6 mobile fixed deadline button grid
st.markdown("""
<style>
...
with tab1:
    ...
    # スマホでも必ず4列×3段になるよう、Streamlit columnsではなく
    # CSS Gridでレイアウトを固定する。クリック自体は透明なStreamlit
    # ボタンをグリッド上に重ね、既存のイベント処理をそのまま使う。
    st.markdown(
        """
        <style>
        .race-deadline-grid{
        ...
        .race-deadline-cell .race-time{
            display:block;
            margin-top:3px;
            font-size:13px;
        }
        .race-deadline-cell.near .race-time{
            color:#e53935;
            font-weight:900;
        }
        @media (max-width:480px){
            .race-deadline-grid{gap:4px;}
            .race-deadline-cell{
                padding:7px 1px;
                font-size:12px;
            }
            .race-deadline-cell .race-time{font-size:12px;}
        }
        </style>
        """,
        unsafe_allow_html=True,
    ...
    for rr in range(1, 13):
        hhmm = deadlines.get(rr, "--:--") if deadlines else "--:--"
        mins = _deadline_minutes_left(d, hhmm)
        near_class = " near" if mins is not None and 0 <= mins <= 30 else ""
        grid_cells.append(
            f'<div class="race-deadline-cell{near_class}">'
            f'<span>{rr}R</span><span class="race-time">{hhmm}</span></div>'
        )
    st.markdown(
        '<div class="race-deadline-grid">' + "".join(grid_cells) + "</div>",
        unsafe_allow_html=True,
    )

    # タップ領域も4列×3段で固定。各ボタンは上のカードと同じ順番。
    # labelにはRと時刻を入れるため、ボタン単体でも操作内容が分かる。
    fetch_requested = False
    for start_r in range(1, 13, 4):
        button_row = st.container(key=f"deadline_button_row_{start_r}")
        cols = button_row.columns(4)
        for col, rr in zip(cols, range(start_r, start_r + 4)):
            hhmm = deadlines.get(rr, "--:--") if deadlines else "--:--"
            is_selected_r = int(st.session_state.get("selected_rno", 12)) == rr
            with col:
                if st.button(
                    f"{rr}R {hhmm}",
                    key=f"race_deadline_btn_{rr}",
                    use_container_width=True,
                    type="primary" if is_selected_r else "secondary",
                ):
                    st.session_state["selected_rno"] = rr
