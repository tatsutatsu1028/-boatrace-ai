...
with tab1:
    ...
    if race is not None:
        ...
        st.markdown("### ④ 選手コメント")
        comments = []
        official_comment_count = 0

        for i in range(6):
            nm = safe_name(edited.iloc[i].get("racer_name", ""))
            official_comment = safe_name(
                edited.iloc[i].get("official_comment", "")
                if "official_comment" in edited.columns
                else ""
            )

            if official_comment:
                official_comment_count += 1

            # レースを変えた時は別keyになるため、そのレースの公式コメントが初期値になります。
            # ユーザーはそのまま使うことも、手入力で修正・追記することもできます。
            txt = st.text_area(
        ...
        if official_comment_count == 6:
            st.success("🗣️ 公式ピットレポート：6艇分のコメントを自動取得しました。")
        elif official_comment_count > 0:
            st.warning(
                f"🗣️ 公式ピットレポート：{official_comment_count}/6艇を取得。"
                "空欄の艇は必要なら手入力してください。"
            )
        else:
            st.info(
                "🗣️ このレースは公式ピットレポート未取得・未掲載です。"
                "必要ならコメントを手入力してください。"
            )
