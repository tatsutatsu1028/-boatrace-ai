...
# v1.10.10 mobile fixed deadline button grid
st.markdown("""
<style>
...
with tab1:
    ...
    # v1.10.10:
    # ページ遷移(href/query parameter)を使わず、Streamlit標準ボタンだけで
    # 4列×3段を作る。これによりタップ時にアプリ全体が再読み込みされず、
    # ログイン画面の一瞬表示・会場選択の初期化を防ぐ。
    st.markdown(
        """
        <style>
        /* race_native_grid の直下に並ぶ12個のボタン要素をCSS Grid化 */
        .st-key-race_native_grid > div > div[data-testid="stVerticalBlock"]{
            display:grid !important;
            grid-template-columns:repeat(4, minmax(0, 1fr)) !important;
            gap:5px !important;
        }
        .st-key-race_native_grid > div > div[data-testid="stVerticalBlock"] > div{
            min-width:0 !important;
            margin:0 !important;
        }
        .st-key-race_native_grid button{
            width:100% !important;
        ...
        .st-key-race_native_grid button p{
            margin:0 !important;
            font-size:11px !important;
    ...
    if near_deadline:
        ...
        for rr in near_deadline:
            _near_css.append(
                f".st-key-race_native_btn_{rr} button p "
                "{color:#e53935 !important;font-weight:900 !important;}"
            )
        _near_css.append("</style>")
        st.markdown("".join(_near_css), unsafe_allow_html=True)

    with st.container(key="race_native_grid"):
        for rr in range(1, 13):
            hhmm = deadlines.get(rr, "--:--") if deadlines else "--:--"
            is_selected_r = current_selected == rr

            with st.container(key=f"race_native_btn_{rr}"):
                if st.button(
                    f"{rr}R\n{hhmm}",
