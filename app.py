...
def _login_gate():
    ...
    if not admin_pin or not staff_pin:
        st.error(
            ...
            "ADMIN_PIN と STAFF_A_PIN を登録してください。"
        )
        st.stop()

    # 24時間ログイン保持用。
    # role/期限だけではURLを書き換えて認証を迂回できるため、HMAC署名も検証する。
    params = st.query_params
    saved_role = str(params.get("login_role", "") or "")
    saved_until = str(params.get("login_until", "") or "")
    saved_sig = str(params.get("login_sig", "") or "")

    def _sign(role, until):
        key = admin_pin if role == "admin" else staff_pin
        payload = f"{role}:{until}".encode("utf-8")
        return hmac.new(key.encode("utf-8"), payload, hashlib.sha256).hexdigest()

    if st.session_state.get("auth_role") not in {"admin", "staff"}:
        try:
            if saved_role in {"admin", "staff"} and saved_until and saved_sig:
                not_expired = int(saved_until) > int(time.time())
                valid_sig = hmac.compare_digest(saved_sig, _sign(saved_role, saved_until))
                if not_expired and valid_sig:
                    ...
                    for key in ("login_role", "login_until", "login_sig"):
                        params.pop(key, None)
    ...
    if st.session_state.get("auth_role") not in {"admin", "staff"}:
        ...
        if st.button("ログイン", key="login_button"):
            ...
            until = str(int(time.time()) + 86400)
            params["login_role"] = role
            params["login_until"] = until
            params["login_sig"] = _sign(role, until)
            st.rerun()
    ...
    if st.button("ログアウト", key="logout_button"):
        for key in ("auth_role", "collector_name", "login_pin"):
            st.session_state.pop(key, None)
        for key in ("login_role", "login_until", "login_sig"):
            params.pop(key, None)
        st.rerun()

    return is_admin, collector_name


IS_ADMIN, COLLECTOR_NAME = _login_gate()
