...
def supabase_config():
    """Supabaseの接続情報を外部（app.py等）から取得するための公開版。"""
...
def save_race_result(
    ...
    # 固定予想（スナップショット）を実際に使えたときだけ snapshot_used を立てる。
    # 「行は存在するが payload が壊れていて復元できなかった」場合に
    # ライブ予想へ黙って戻りながら「固定予想で判定」と表示してしまうと、
    # この仕組みの唯一の保証が嘘になるため、復元成功と行の存在を区別する。
    snapshot_used = None
    if prefer_snapshot:
        snapshot_row = snapshot if snapshot is not None else load_prediction_snapshot(race_key)
        ...
        if (
            ...
            and len(snap_final)
            and "lane" in snap_final.columns
            and "p_first" in snap_final.columns
            and snap_tickets is not None
            and len(snap_tickets)
            and {"combo", "stake"}.issubset(snap_tickets.columns)
        ):
            final = snap_final
            tickets = snap_tickets
            research_variants = snap_research
            snapshot_used = snapshot_row
        elif require_snapshot:
            raise ValueError(
                "固定予想を復元できなかったため、検証結果を保存しませんでした。"
                "レース後の再予想が混ざるのを防ぐための安全策です。"
            )
...
DEFAULT_SETTINGS = {
    "main_n": 3,
    "cover_n": 3,
    # 穴は既定で買わない。検証111レースで穴は106点・13,200円を
    # 購入して的中ゼロ、回収率0%だった（前半・後半のどちらも0%）。
    # 平均的中確率が0.9〜2.0%しかなく、控除率25%の中でこの帯を
    # 買い続けて勝つのは構造的に難しい。
    "hole_n": 0,
    "total_budget": 2000,
    "min_bet": 100,
...
def save_odds_snapshot_now(race_date, jcd, rno, odds_df):
    """現在取得済みの3連単オッズを、その場でodds_snapshotsへ1時点分保存する。"""
    if not _use_supabase():
        return False, "Supabaseが設定されていないため、オッズを保存できません。"

    if odds_df is None or len(odds_df) == 0:
        return False, "現在の3連単オッズがまだ取得できていません。"
    ...
    try:
        ...
        if not payload:
            return False, "保存できるオッズがありませんでした。"

        endpoint = f"{url}/rest/v1/{SUPABASE_ODDS_TABLE}"
        r = requests.post(
            endpoint,
        ...
        r.raise_for_status()
        return True, f"最初のオッズを即保存しました（{len(payload)}通り）。"
    except Exception as e:
        print("[RESULT_TRACKER] immediate odds save error:", type(e).__name__, str(e), flush=True)
        return False, f"最初のオッズ保存に失敗しました: {e}"



def snapshot_payout_from_official(race_key, actual_combo, payout_per_100, snapshot=None):
    """
    固定済み予想の購入額を使って実受取額を算出する。

    公式の3連単払戻は100円あたりなので、
    的中買い目の固定時購入額 / 100 × 公式払戻金 とする。

    固定予想を読み込めなかった場合・payloadが壊れていた場合は
    ValueError を送出する。以前はこの場合も (0, 0) を返していたが、
    それでは「通信に失敗した」と「買い目が的中しなかった」が区別できず、
    的中したレースを収支マイナスとして保存してしまうため。

    snapshot を渡すと再取得せずそれを使う。呼び出し側で1回だけ読み込み、
    払戻計算と結果保存で同じスナップショットを使うことで、
    2回の通信結果が食い違うのを防ぐ。
    """
    snapshot = snapshot if snapshot is not None else load_prediction_snapshot(race_key)
    if not snapshot:
        raise ValueError(
            "固定予想を読み込めませんでした。"
    ...
    raw = snapshot.get("payload_json", "")
    if not raw:
        raise ValueError("固定予想の中身が空でした。")

    try:
        payload = json.loads(raw)
    except Exception:
        raise ValueError("固定予想の中身を解析できませんでした。")

    if not isinstance(payload, dict):
        raise ValueError("固定予想の形式が不正です。")
    ...
    try:
        payout100 = int(float(payout_per_100))
    except Exception:
        payout100 = 0
...
def deactivate_odds_watchlist(race_date, jcd, rno):
    """結果確定後、このレースの定期オッズ追跡を停止する。"""
    if not _use_supabase():
        return False
    ...
    try:
        url, _ = _supabase_config()
        endpoint = (
            f"{url}/rest/v1/{SUPABASE_WATCHLIST_TABLE}"
            f"?race_date=eq.{requests.utils.quote(str(race_date), safe='')}"
            f"&jcd=eq.{requests.utils.quote(str(jcd).zfill(2), safe='')}"
            f"&rno=eq.{int(rno)}"
        )
        r = requests.patch(
            endpoint,
        ...
        print(
            "[RESULT_TRACKER] watchlist deactivate error:",
            type(e).__name__,
            str(e),
            flush=True,
        )
