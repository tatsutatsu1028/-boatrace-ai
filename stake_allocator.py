...
def allocate_stakes_smart(
    ...
    # -----------------------------------------------------------
    # use_odds=False（既定）: オッズを配分判断に使わない
    # -----------------------------------------------------------
    # 検証318レース・2,461点で、同じ買い目・同じ総額のまま配分方法だけを
    # 変えて比較したところ、オッズを重視するほど成績が単調に悪化した。
    #
    #   確率比例（オッズ無視） 91.0%   前半93.5% / 後半88.6%
    #   現行（EV考慮）         86.8%
    #   EV比例                 81.0%
    #   EV超過分に比例         72.7%
    #   均等配分               77.9%
    #
    # 均等配分より確率比例が良いので、確率の情報自体には価値がある。
    # 壊しているのはオッズを掛け合わせる操作の方。
    #
    # 理由: 期待値は「確率 × オッズ」だが、この確率には較正後もなお
    # 誤差が残る。300倍の買い目で確率を2倍見誤ると期待値も2倍ずれるため、
    # 期待値で順位付けすると「モデルの誤差が最大の買い目」が上位に来る。
    # 一方オッズは大勢の投票で決まる価格で、こちらのモデルより正確。
    # 実測でもオッズ200倍超は48点・的中0・回収率0%（前半後半とも0%）。
    #
    # 注意: これは損失を減らす措置であって、勝つための変更ではない。
    # 市場に勝つには「オッズが示す確率よりモデルが正確な場面」を
    # 見つける必要があり、それには必ずオッズが要る。モデルに優位が
    # 確認できた時点で use_odds=True に戻して再評価すること。
    if use_odds:
        # 従来どおり。sqrt(=0.5)は本線偏重寄り。
        prob_exponent = 0.5 - 0.15 * value_bias
    else:
        # 検証で最も成績が良かった「確率に比例」＝指数1.0。
        prob_exponent = 1.0
    ...
    for _, row in df.iterrows():
        ...
        # EV補正（use_odds=False のときは一切かけない）
        if not use_odds:
            ev_mult = 1.0
        elif math.isnan(ev):
            ev_mult = 0.80
