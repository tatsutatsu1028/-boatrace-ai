...
def allocate_stakes_smart(
    ...
    ev_floor / hard_ev_cut : float
        以前はev_floor=0.85, hard_ev_cut=0.55だった。しかし検証57
        レースの実際の的中チケットを調べると、的中した9点のうち7点が
        EV0.78〜0.98（＝ev_floor未満）で、EV1未満というだけで
        0.45倍まで減額されていた。3連単の本命絡み（人気）の買い目は
        オッズが低くEVが1を割りやすい構造上、これは「人気で当たり
        やすい買い目」を実際より不当に減額していたことになる。
        ev_floor/hard_ev_cutを引き下げてこの減額を緩和したところ、
        同じ57レースの買い目・オッズで再計算した回収率が19.7%→22.4%
        に改善した（母数は少なく参考値）。
    """
