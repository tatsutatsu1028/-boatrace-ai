...
def fetch_race_result(date_yyyymmdd, jcd, rno):
    ...
    結果未確定・中止・返還等で通常の3連単結果を確定できない場合は
    ValueError とし、UI側では手入力にフォールバックできるようにする。
    """
    ...
    # 「着 / 枠 / ボートレーサー / レースタイム」の結果表を優先して読む。
    for table in soup.find_all("table"):
        grid = _expand_table(table)
        if not grid:
            continue

        blob = " ".join(" ".join(r) for r in grid[:3])
        ...
        for row in grid[1:]:
            ...
            # 全角数字は _norm() 内の NFKC で半角化される。
            rank_m = re.fullmatch(r"([1-6])", rank_txt)
            lane_m = re.fullmatch(r"([1-6])", lane_txt)
    ...
    if not all(k in finishes for k in (1, 2, 3)):
        raise ValueError("公式結果がまだ確定していないか、通常着順を取得できません。")

    first = int(finishes[1])
    second = int(finishes[2])
    third = int(finishes[3])
    combo = f"{first}-{second}-{third}"

    trifecta_combo = None
    trifecta_payout = None

    # 「勝式 / 組番 / 払戻金 / 人気」の払戻表から3連単を読む。
    for table in soup.find_all("table"):
        grid = _expand_table(table)
        for row in grid:
            if not row:
    ...
    if trifecta_payout is None:
        raise ValueError("公式結果から3連単払戻金を取得できませんでした。")

    # 着順と払戻表の組番が食い違う場合は自動入力しない。
    if trifecta_combo and trifecta_combo != combo:
        raise ValueError(
            f"着順({combo})と3連単払戻表({trifecta_combo})が一致しません。"
        )
