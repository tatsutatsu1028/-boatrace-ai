...
def fetch_race_result(date_yyyymmdd, jcd, rno):
    """
    BOAT RACE公式の結果ページから、1～3着と3連単払戻（100円あたり）を取得する。

    Phase 2 result fix:
    公式結果ページでは会場・タイミングによって着順表のHTML構造が微妙に異なる
    ことがあるため、まず払戻表の「3連単 組番」を確定結果として取得する。
    3連単の組番自体が 1着-2着-3着 を表すので、そこから着順を復元する。

    着順表も読めた場合だけ整合性チェックを行い、着順表が読めないことだけを
    理由に自動保存を止めない。
    """
    html, url = _get("raceresult", date_yyyymmdd, jcd, rno)
    soup = BeautifulSoup(html, "lxml")

    # -------------------------------------------------
    # 1) 払戻表から3連単の組番・払戻を先に取得する
    # -------------------------------------------------
    trifecta_combo = None
    ...
    for table in soup.find_all("table"):
        ...
        for row in grid:
            ...
            if not any("3連単" in c for c in cells):
                continue
            ...
            for i, c in enumerate(cells):
                ...
                if m:
                    trifecta_combo = "-".join(m.groups())
                    combo_index = i
                    break

            if trifecta_combo is None:
                # table展開の都合で組番が複数セルに分かれた場合への保険。
                joined = " ".join(cells)
                m = re.search(
            ...
            # 組番より右側を優先し、¥1,230 / ￥1,230 / 1,230円 等を読む。
            payout_candidates = (
                cells[combo_index + 1:]
            ...
            for value in payout_candidates:
                cleaned = (
                    value.replace("¥", "")
                    .replace("￥", "")
                    .replace("円", "")
                ...
                if re.fullmatch(r"\d+", cleaned):
                    amount = int(cleaned)
                    # 人気順位などの小さな整数を誤って払戻にしないため、
                    # 3連単払戻として成立する100円以上を採用。
                    if amount >= 100:
        ...
        if trifecta_combo is not None and trifecta_payout is not None:
            break

    if trifecta_combo is None or trifecta_payout is None:
        raise ValueError(
            "公式結果から3連単の組番または払戻金を取得できませんでした。"
        )

    combo_parts = trifecta_combo.split("-")
    if len(combo_parts) != 3:
        raise ValueError("公式結果の3連単組番を正しく解析できませんでした。")

    first, second, third = [int(v) for v in combo_parts]
    ...
    for table in soup.find_all("table"):
        ...
        blob = " ".join(" ".join(r) for r in grid[:4])
        if "着" not in blob or "枠" not in blob:
            continue
        ...
        for row in grid:
            ...
            rank_txt = _norm(row[0])
            lane_txt = _norm(row[1])

            rank_m = re.fullmatch(r"\s*([1-6])\s*", rank_txt)
            lane_m = re.fullmatch(r"\s*([1-6])\s*", lane_txt)

            if rank_m and lane_m:
                finishes[int(rank_m.group(1))] = int(lane_m.group(1))

        if all(k in finishes for k in (1, 2, 3)):
            break

    # 着順表が3着まで読めた場合だけ、3連単組番と照合する。
    if all(k in finishes for k in (1, 2, 3)):
        finish_combo = f"{finishes[1]}-{finishes[2]}-{finishes[3]}"
        if finish_combo != combo:
            raise ValueError(
                f"着順表({finish_combo})と3連単払戻表({combo})が一致しません。"
            )
