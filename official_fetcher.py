...
def _strip_comment_confidence(text):
    s = _norm(text)
    # 全角/半角括弧・星表記の揺れに対応
    s = re.sub(
        r"[（(]\s*コメント自信度\s*[・･\.…]*\s*[★☆＊*]*\s*[）)]",
...
def _comment_confidence(text):
    """
    公式の「コメント自信度・・★★☆」を 0〜3 の星数として保存。
    表記が無い場合は NaN。
    """
    s = _norm(text)
    m = re.search(r"コメント自信度\s*[・･\.…]*\s*([★☆＊*]{1,3})", s)
    if not m:
        return np.nan
    stars = m.group(1)
    return float(stars.count("★") + stars.count("*") + stars.count("＊"))

def _pitreport_rows(table):
    """
    pitreport の各艇行を拾う。
    公式ページは概ね
    [枠, 写真, レーサー情報, ピットレポート, 前走結果]
    の構成。colspan/rowspan は _expand_table で吸収。
    """
    rows = {}
    for row in _expand_table(table):
        if not row:
...
def _parse_pitreport_row(ln, row):
    rec = {
        "lane": ln,
        "official_comment": "",
        "comment_confidence": np.nan,
    }
    if not row:
        return rec
    ...
    rec["racer_name_pitreport"] = _racer_name(row[racer_i])

    # レーサー情報セルより後ろから、最も「コメントらしい」長文を選ぶ。
    # 前走結果の "4R" 等を誤採用しないよう長さと日本語率を重視。
    candidates = []
    for i, c in enumerate(row[racer_i + 1:], start=racer_i + 1):
        s = _norm(c)
        if not s:
            continue
        if re.fullmatch(r"\d{1,2}R", s):
            continue
        if s in {"ピットレポート", "前走", "結果"}:
            continue

        jp_chars = len(re.findall(r"[一-龥々ぁ-んァ-ヶー]", s))
        score = len(s) + jp_chars * 2

        if "コメント自信度" in s:
            score += 100
        if any(w in s for w in (
            "足", "伸び", "出足", "回り", "乗り", "ペラ", "調整",
            "スタート", "ターン", "直線", "レース", "行き足", "気配"
        )):
            score += 25
    ...
    if candidates:
        _, _, raw = max(candidates, key=lambda x: x[0])
        rec["comment_confidence"] = _comment_confidence(raw)
        rec["official_comment"] = _strip_comment_confidence(raw)

    return rec

def fetch_pitreport(date_yyyymmdd, jcd, rno):
    """
    BOAT RACE公式「ピットレポート」から6艇のコメントを取得。
    レース・開催によって未掲載の場合は空欄で返す。
    """
    html, url = _get("pitreport", date_yyyymmdd, jcd, rno)
    soup = BeautifulSoup(html, "lxml")

    table = _find_table(
        soup,
        ["ボートレーサー", "ピットレポート", "コメント自信度"]
    )

    records = {}
    if table is not None:
        for ln, row in _pitreport_rows(table).items():
            records[ln] = _parse_pitreport_row(ln, row)
    ...
    # 念のため6艇・lane固定
    if "official_comment" not in out.columns:
        out["official_comment"] = ""
    if "comment_confidence" not in out.columns:
        out["comment_confidence"] = np.nan

    out["official_comment"] = out["official_comment"].fillna("").astype(str)
    out["source_pitreport"] = url
    return out.sort_values("lane").reset_index(drop=True)
...
def fetch_official_race(date_yyyymmdd, jcd, rno):
    ...
    print("[FETCH_TIME] START", "date=", date_yyyymmdd, "jcd=", jcd, "rno=", rno)
    try:
        _t = time.perf_counter()
        base = fetch_racelist(date_yyyymmdd, jcd, rno)
        print(f"[FETCH_TIME] racelist: {time.perf_counter() - _t:.2f}s")
    ...
    try:
        ...
        print(f"[FETCH_TIME] current_meet: {time.perf_counter() - _t:.2f}s")
    ...
    try:
        ...
        print(f"[FETCH_TIME] course_stats: {time.perf_counter() - _t:.2f}s")
    ...
    try:
        ...
        print(f"[FETCH_TIME] beforeinfo: {time.perf_counter() - _t:.2f}s")
    except Exception as e:
        raise RuntimeError(
    ...
    try:
        _t = time.perf_counter()
        pit = fetch_pitreport(date_yyyymmdd, jcd, rno)
        print(f"[FETCH_TIME] pitreport: {time.perf_counter() - _t:.2f}s")
    except Exception:
        pit = pd.DataFrame({
            "lane": range(1, 7),
            "official_comment": [""] * 6,
            "comment_confidence": [np.nan] * 6,
            "source_pitreport": [""] * 6,
        })
    ...
    try:
        ...
        print(f"[FETCH_TIME] venue_comments: {time.perf_counter() - _t:.2f}s")
    ...
    try:
        ...
        out = out.merge(
            venue_comments.drop(columns=["racer_name"], errors="ignore"),
            on="lane",
            how="left",
        )
        ...
        # コメント優先順位:
        # 1) 場公式の独自選手コメント
        # 2) BOAT RACE本体 pitreport
        # 3) アプリ上で手入力
        for c in ("venue_comment", "venue_comment_source", "venue_comment_url"):
        ...
        out["auto_comment"] = out["venue_comment"].where(
            out["venue_comment"].str.strip() != "",
            out["official_comment"],
        )
        out["auto_comment_source"] = out["venue_comment_source"].where(
            out["venue_comment"].str.strip() != "",
            np.where(
                out["official_comment"].str.strip() != "",
                "BOAT RACE公式・ピットレポート",
                "",
            ),
        )
        ...
        print(f"[FETCH_TIME] TOTAL: {time.perf_counter() - _t_total:.2f}s")
        return out.sort_values("lane").reset_index(drop=True)
