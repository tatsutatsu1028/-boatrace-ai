...
from original_exhibition import fetch_original_exhibition
...
def fetch_official_race(date_yyyymmdd, jcd, rno):
    ...
    今節成績・コース適性・直前情報・ピットレポート・場コメントを
    並列取得する。
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = {
            ...
            "venue_comments": executor.submit(
                _timed,
                "venue_comments",
                fetch_venue_comments,
                date_yyyymmdd,
                jcd,
                rno,
                base.copy(),
            ),
            "original_exhibition": executor.submit(
                _timed,
                "original_exhibition",
                fetch_original_exhibition,
                date_yyyymmdd,
                jcd,
                rno,
                base.copy(),
        ...
        # 場コメントも失敗しても続行
        try:
            venue_comments = futures["venue_comments"].result()
        ...
        # オリジナル展示（直線・まわり足・1周）も失敗しても続行
        try:
            original_exhibition = futures["original_exhibition"].result()
            ...
            print(
                "[OFFICIAL_FETCHER] original exhibition done:",
                "rows=", len(original_exhibition),
                "got=", got,
                flush=True,
            )
        except Exception as e:
            print(
                "[OFFICIAL_FETCHER] original exhibition ERROR:",
                type(e).__name__,
                str(e),
                flush=True,
            )
